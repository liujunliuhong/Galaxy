...
public class GLTabBar: UITabBar {
    ...
    private var beforSelectIndex: Int = 0
    private let baseTag: Int = 1000
    private var wrapViews: [_GLabBarItemWrapView] = []
...
extension GLTabBar {
    private func updateDisplay() {
        ...
        for (index, item) in tabBarItems.enumerated() {
            let wrapView = _GLabBarItemWrapView(target: self)
            wrapView.tag = baseTag + index
...
extension GLTabBar {
    @objc internal func selectAction(_ sender: AnyObject?) {
        guard let v = sender as? _GLabBarItemWrapView else {
            return
        }
